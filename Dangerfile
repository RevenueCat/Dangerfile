...
def fail_if_no_supported_label_found
  ...
  | *pr:changelog_ignore* | The PR will not be included in the changelog. This label doesn't determine the type of bump of the version and needs can be combined with `pr:feat`, `pr:fix` or `pr:other`. |
    MARKDOWN
  end
end
